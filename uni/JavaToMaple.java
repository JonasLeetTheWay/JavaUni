...
public class JavaToMaple{
	 ...
	 *  to the console
	 */
	public static void printToConsole(double dt){
		System.out.println(dt +", "+ x[x.length-1]/x.length +", "+ y[y.length-1]/y.length);
	}
	...
	public static void calc(double v0, double dt, double ang){
		...
		for(int i=0; i<t.length; i++){
			ti += dt; // time at every step
			x[i] = Math.rint( v0 * ti * Math.cos(rad) );
			y[i] = Math.rint( v0 * ti * Math.sin(rad) - 0.5 * G * ti*ti );
		}
	}
